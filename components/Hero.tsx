      ...
      <div className="container mx-auto px-6 grid lg:grid-cols-12 gap-12 items-center relative z-10">
        <div className="lg:col-span-6 order-2 lg:order-1 space-y-10 animate-fade-in-up text-center lg:text-left pt-10 lg:pt-0">
          <div className="flex items-center justify-center lg:justify-start gap-4">
            ...
            <span className="text-brand-gold/80 text-[10px] font-display tracking-[0.4em] uppercase">{c('hero.eyebrow')}</span>
          ...
          <div className="space-y-4">
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-display text-brand-cream leading-[0.85] tracking-tight">
              {c('hero.line1')} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-gold via-[#FFF5D6] to-brand-gold italic pr-4 font-serif relative z-10 filter drop-shadow-sm">{c('hero.line2')}</span>
            </h1>
          </div>

          <p className="text-lg text-brand-cream/80 font-serif leading-relaxed max-w-lg mx-auto lg:mx-0 border-l border-brand-gold/30 pl-6">{c('hero.slogan')}</p>
          <p className="text-base text-brand-cream/60 font-sans leading-relaxed max-w-lg mx-auto lg:mx-0">{c('hero.subline')}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center lg:justify-start pt-4">
            <Button onClick={onOrderClick} className="bg-gradient-to-r from-brand-gold to-[#B8860B] text-brand-maroon hover:to-brand-gold shadow-glow-gold border-none">
              {ordering.enabled ? 'Commander' : 'Découvrir'} <ArrowRight size={18} />
            </Button>
        ...
        <div className="lg:col-span-6 order-1 lg:order-2 relative flex justify-center items-center">
          <div className="relative w-full max-w-[450px] aspect-[4/5]">
            ...
            <div className="relative h-full w-full arch-mask shadow-2xl bg-black group">
              <img src={c('hero.image')} alt="Les Saveurs du Maghreb - Restaurant Maghrébin à Nancy" className="w-full h-full object-cover opacity-90 group-hover:scale-110 transition-transform duration-[8s] ease-out" />
